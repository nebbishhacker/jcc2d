...
Tracks::Tracks(double x, double y) : PhysicsSprite("images/spr_enemy9.png")
{
	setPosition(x, y);
...
void Tracks::initialize() // use initialize for anything involving world (like groups)
{
...
void Tracks::update()
{
	...
	if (walking) // If we are walking:
	{
		if (!flipped)	// and facing left (the image faces right)
			contactVelocity.x += 1.75; // move feet right (thus pushing self left);
		else			// and facing right
			contactVelocity.x -= 1.75; // move feet left (thus pushing self right);
	}
